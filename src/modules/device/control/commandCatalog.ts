...
const commands: Record<DeviceType, DeviceCommandSpec[]> = {
  Access: [
    ...
    { commandLine: 'CLOSE_ACCESS_ONCE', label: '单次关门', description: '立即关闭门禁一次。', tone: 'caution', inputs: [] },
    {
      ...
      label: '设置关门延时',
      description: '设置门禁自动关门前的等待时间，最多 255 秒。',
      tone: 'normal',
      inputs: [{ name: 'delay', label: '延时时间', unit: '秒', min: 0, max: 255 }],
    },
  ...
  AirCondition: [
    ...
    { commandLine: 'CLOSE_AIR_CONDITION_RS485', label: '关闭空调', description: '关闭目标空调。', tone: 'caution', inputs: [] },
    {
      ...
      label: '增强控制',
      description: '一次调整开关、模式、温度和风速；不需要修改的项目可以保持不变。',
      tone: 'normal',
      inputs: [
        { name: 'opened', label: '运行状态', options: [keep, { value: 1, label: '开启' }, { value: 0, label: '关闭' }] },
        { name: 'mode', label: '运行模式', options: [keep, { value: 1, label: '制热' }, { value: 2, label: '制冷' }, { value: 4, label: '送风' }, { value: 8, label: '除湿' }] },
        { name: 'temperature', label: '设定温度', options: [keep, ...Array.from({ length: 15 }, (_, index) => ({ value: index + 16, label: `${index + 16}°C` }))] },
        { name: 'speed', label: '风速', options: [keep, { value: 0, label: '自动' }, { value: 1, label: '低速' }, { value: 2, label: '中速' }, { value: 3, label: '高速' }] },
      ],
    },
